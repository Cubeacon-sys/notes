...
\subsubsection{Implikasi Kondisional}
Bentuk ini menyatakan hubungan syarat (anteseden) dan konsekuensi (konsekuen).
\begin{itemize}
...
\item \textbf{Aturan Negasi (Hukum Negasi Implikasi): } \[ \neg (p \implies q) \equiv p \land \neg q \]
\textit{(Perhatian: Pengecoh sering menyatakan negasinya adalah $\neg p \implies \neg q$, yang mana bernilai salah secara logika.)}
\end{itemize}
...
\subsubsection{Kuantor Bersyarat Kompleks}
Seringkali semesta pembicaraan memiliki dua syarat (konjungsi) sebelum dikenai konsekuensi.
\begin{itemize}
\item \textbf{Ekstraksi Kalimat: } ``Semua $A$ yang juga $B$ memiliki sifat $C$''.
\item \textbf{Notasi Logika Predikat: } \[ \forall x \, ((A(x) \land B(x)) \implies C(x)) \]
\item \textbf{Aturan Negasi: } \[ \neg \big[ \forall x \, ((A(x) \land B(x)) \implies C(x)) \big] \equiv \exists x \, (A(x) \land B(x) \land \neg C(x)) \]
\textit{(Dibaca: ``Ada A yang juga B, tetapi BUKAN C'')}
\end{itemize}

\subsection{Aturan Inferensi (Mesin Penarikan Kesimpulan)}
Gunakan skema deduktif formal di bawah ini. Jika premis soal dapat disusun menjadi bentuk di atas garis, maka kesimpulan di bawah tanda $\therefore$ adalah jawaban mutlak yang valid.
